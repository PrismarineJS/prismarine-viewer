...
function setupRoutes(app, prefix = "") {
  ...
  app.get("/skin", async (req, res) => {
    ...
    try {
      let uuid = await fetch(`https://api.mojang.com/users/profiles/minecraft/${username}?at=${Date.now()}`)
        .then((r) => r.json())
        .then((r) => r.id)
      let r = await fetch(`https://sessionserver.mojang.com/session/minecraft/profile/${uuid}`).then((r) => r.json())
      let skinData = JSON.parse(Buffer.from(r.properties[0].value, "base64").toString("utf8"))
      let url = skinData.textures.SKIN.url.replace("http://", "https://")
